...
protected:
  ...
  /** \brief Templatized read function for reading a single element of a given
   * type.
   */
  template <class T>
   ...
   *
   * This function will ensure that the correct number of words are transferred
   * from the FPGA for any specified size. Transferred data should be allocated
   * using the App::malloc function.
   */
  template <class T>
  void read(T *data, unsigned long addr, unsigned long size,
            MemId memId = MEM_INTERNAL);

  /** \brief Templatized write function for writing a single element of a given
   * type.
   */
  template <class T>
   ...
   *
   * This function will ensure that the correct number of words are transferred
   * to the FPGA for any specified size. Transferred data should be allocated
   * using the App::malloc function.
   */
  template <class T>
  void write(const T *data, unsigned long addr, unsigned long size,
             MemId memId = MEM_INTERNAL);
};

template <class T> void App::read(T &data, unsigned long addr, MemId memId) {

  unsigned long numWords =
      (unsigned long)ceil(sizeof(T) / (float)sizeof(boardWord_t));
...
void App::read(T *data, unsigned long addr, unsigned long size, MemId memId) {
  ...
  unsigned long numWords =
      (unsigned long)ceil(size * sizeof(T) / (float)sizeof(boardWord_t));
...
void App::write(const T &data, unsigned long addr, MemId memId) {
  ...
  delete[] temp;
  if (!ok)
    throw "Failure in App::write()";
}

template <class T>
void App::write(const T *data, unsigned long addr, unsigned long size,
                MemId memId) {
