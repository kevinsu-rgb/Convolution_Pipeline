...
Board::Board(const char *bitfile, const vector<float> &clocks)
    : PAGE_SIZE(sysconf(_SC_PAGESIZE)) {

  if (clocks.size() != NUM_FPGA_CLOCKS) {

    ostringstream errorMsg;
    errorMsg << "Error: clocks vector must have " << NUM_FPGA_CLOCKS
             << " frequencies.";
...
void Board::configureFpgaClock(unsigned clockId, double freq) {
  ...
  // printf("%s %s %s %s %s %s\n", name.str().c_str(), path.str().c_str(),
  // freqFile.str().c_str(), enableFile.str().c_str(), freqData.str().c_str(),
  // enableData.c_str());
...
void Board::copy(const char *to, const char *from) {
  ...
  if (inFile == NULL) {
    handleError("Error opening " + (string)from);
  }
  ...
  if (result != lSize) {
    handleError("Error reading " + (string)from);
  }

  // open and write to output file
  outFile = fopen(to, "w");
  if (outFile == NULL) {
    handleError("Error opening " + (string)to);
  }
...
void Board::loadBitfile(const char *bitfile) { copy("/dev/xdevcfg", bitfile); }
...
void Board::initializeMemoryMap() {
  ...
  for (unsigned i = 0; i < numPages; i++) {
    ...
    void *ptr =
        mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, pageAddr);
...
inline bool Board::write(unsigned *data, unsigned long addr,
                         unsigned long words) {
  // identify the starting page, the number of pages for the
  // entire transfer, and the addr within the starting page
  unsigned page = addr * sizeof(boardWord_t) / PAGE_SIZE;
  unsigned pages = ceil(words * sizeof(boardWord_t) / (float)PAGE_SIZE);
  addr = addr % (PAGE_SIZE / sizeof(boardWord_t));

  // for each page, transfer the corresponding data
  for (unsigned i = 0; i < pages; i++) {

    unsigned pageWords = words > PAGE_SIZE / sizeof(boardWord_t)
                             ? PAGE_SIZE / sizeof(boardWord_t) - addr
                             : words;
    // memcpy(mmapPages[page]+addr, data+i*PAGE_SIZE/sizeof(boardWord_t),
    // pageWords*sizeof(unsigned));

    for (unsigned j = 0; j < pageWords; j++) {
      unsigned *dst = mmapPages[page] + addr + j;
      unsigned *src = data + i * PAGE_SIZE / sizeof(boardWord_t) + j;
      *dst = *src;
    }
...
inline bool Board::read(unsigned *data, unsigned long addr,
                        unsigned long words) {
  // identify the starting page, the number of pages for the
  // entire transfer, and the addr within the starting page
  unsigned page = addr * sizeof(boardWord_t) / PAGE_SIZE;
  ...
  for (unsigned i = 0; i < pages; i++) {
    ...
    unsigned pageWords = words > PAGE_SIZE / sizeof(boardWord_t)
                             ? PAGE_SIZE / sizeof(boardWord_t) - addr
                             : words;
    // memcpy(data+i*PAGE_SIZE/sizeof(boardWord_t), mmapPages[page]+addr,
    // pageWords*sizeof(unsigned));

    for (unsigned j = 0; j < pageWords; j++) {
      unsigned *src = mmapPages[page] + addr + j;
      unsigned *dst = data + i * PAGE_SIZE / sizeof(boardWord_t) + j;
      *dst = *src;
    }
