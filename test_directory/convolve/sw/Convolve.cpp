...
Kernel::Kernel(const appWord_t *kernel, unsigned int size) {
  ...
  if (size > Convolve::MAX_KERNEL_SIZE) {
    cerr << "Current FPGA implemenetation doesn't support kernels larger than "
         << Convolve::MAX_KERNEL_SIZE << endl;
...
unsigned int Kernel::getSize() const { return size; }

unsigned int *Kernel::getKernel() const { return kernel; }
...
Signal::Signal(const appWord_t *signal, unsigned int size) {
  ...
  for (unsigned i = 0, j = 0; i < this->size; i++) {
    ...
    if (i < Convolve::MAX_KERNEL_SIZE - 1 ||
        i >= size + Convolve::MAX_KERNEL_SIZE - 1) {
      this->signal[i] = 0;
    } else {
...
unsigned int Signal::getSize() const { return size; }

unsigned int Signal::getUnpaddedSize() const { return unpaddedSize; }

appWord_t *Signal::getSignal() const { return signal; }
