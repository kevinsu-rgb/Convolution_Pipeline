...
void convolveSW(const unsigned short *input, unsigned int inputSize,
  ...
  for (i = 0; i < outputSize; i++) {
    for (j = 0; j < kernelSize; j++) {
      ...
      sum = product + (unsigned int)output[i] > 0xffff ? 0xffff
                                                       : product + output[i];
      output[i] = sum;
    }
  }
...
bool convolveHW(Convolve &convolve, const unsigned short *input,
                unsigned int inputSize, const unsigned short *kernel,
                unsigned int kernelSize, unsigned short *output) {
...
bool test(Convolve &convolve, const unsigned short *input,
          unsigned int inputSize, const unsigned short *kernel,
          unsigned int kernelSize, unsigned short *swOutput,
          unsigned short *hwOutput, float &percentCorrect, float &speedup) {
...
void testZeros(Convolve &convolve, unsigned short *input,
               unsigned int inputSize, unsigned short *kernel,
               unsigned int kernelSize, unsigned short *swOutput,
               unsigned short *hwOutput, float &percentCorrect,
               float &speedup) {
  ...
  test(convolve, input, inputSize, kernel, kernelSize, swOutput, hwOutput,
       percentCorrect, speedup);
}

void testOnes(Convolve &convolve, unsigned short *input, unsigned int inputSize,
...
void testRandNoClip(Convolve &convolve, unsigned short *input,
                    unsigned int inputSize, unsigned short *kernel,
                    unsigned int kernelSize, unsigned short *swOutput,
                    unsigned short *hwOutput, float &percentCorrect,
                    float &speedup) {
...
void testRand(Convolve &convolve, unsigned short *input, unsigned int inputSize,
              unsigned short *kernel, unsigned int kernelSize,
              unsigned short *swOutput, unsigned short *hwOutput,
              float &percentCorrect, float &speedup) {
...
int main(int argc, char *argv[]) {
  ...
  transferSize = App::getSafeTransferSize(Convolve::MAX_SIGNAL_SIZE,
                                          sizeof(unsigned short));
  input = new unsigned short[transferSize];
  transferSize = App::getSafeTransferSize(Convolve::MAX_KERNEL_SIZE,
                                          sizeof(unsigned short));
  kernel = new unsigned short[transferSize];
  transferSize = App::getSafeTransferSize(Convolve::MAX_OUTPUT_SIZE,
                                          sizeof(unsigned short));
  ...
  testZeros(convolve, input, SMALL_SIGNAL, kernel, SMALL_KERNEL, swOutput,
            hwOutput, percentCorrect, speedup);
  ...
  testOnes(convolve, input, SMALL_SIGNAL, kernel, SMALL_KERNEL, swOutput,
           hwOutput, percentCorrect, speedup);
  ...
  cout << "Testing small signal/kernel with random values (no clipping)..."
       << endl;

  testRandNoClip(convolve, input, SMALL_SIGNAL, kernel, SMALL_KERNEL, swOutput,
                 hwOutput, percentCorrect, speedup);
  ...
  cout << "Testing medium signal/kernel with random values (no clipping)..."
       << endl;

  testRandNoClip(convolve, input, MEDIUM_SIGNAL, kernel, MEDIUM_KERNEL,
                 swOutput, hwOutput, percentCorrect, speedup);
  ...
  cout << "Testing big signal/kernel with random values (no clipping)..."
       << endl;

  testRandNoClip(convolve, input, BIG_SIGNAL, kernel, BIG_KERNEL, swOutput,
                 hwOutput, percentCorrect, speedup);
  ...
  testRand(convolve, input, SMALL_SIGNAL, kernel, SMALL_KERNEL, swOutput,
           hwOutput, percentCorrect, speedup);
  ...
  testRand(convolve, input, MEDIUM_SIGNAL, kernel, MEDIUM_KERNEL, swOutput,
           hwOutput, percentCorrect, speedup);
  ...
  testRand(convolve, input, BIG_SIGNAL, kernel, BIG_KERNEL, swOutput, hwOutput,
           percentCorrect, speedup);
